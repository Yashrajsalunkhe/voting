...
import { 
  ...
  CandidateModel, 
  VoteModel,
  SecondYearVotesModel,
  ThirdYearVotesModel,
  FinalYearVotesModel,
  type Student, 
  type Candidate, 
  type Vote, 
  type YearVoteResponse,
  type InsertStudent, 
  type InsertVote, 
  Position 
} from "./shared/schema";
...
export class MongoStorage implements IStorage {
  ...
  async submitVotes(votesToSubmit: InsertVote[]): Promise<void> {
  ...
  private async saveVotesToYearCollections(votesToSubmit: InsertVote[]): Promise<void> {
  ...
  async getSecondYearVotes(): Promise<YearVoteResponse[]> {
    return await SecondYearVotesModel.find().lean();
  }

  async getThirdYearVotes(): Promise<YearVoteResponse[]> {
    return await ThirdYearVotesModel.find().lean();
  }

  async getFinalYearVotes(): Promise<YearVoteResponse[]> {
    return await FinalYearVotesModel.find().lean();
  }

  async getVotesByYear(year: 'second' | 'third' | 'final'): Promise<YearVoteResponse[]> {
    switch (year) {
      case 'second':
  ...
  async getYearBasedResults(year: 'second' | 'third' | 'final'): Promise<{ position: string; candidateName: string; urn: string; votedAt: Date }[]> {
