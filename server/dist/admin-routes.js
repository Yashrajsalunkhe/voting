...
const adminAuth = (req, res, next) => {
    ...
    if (!adminKey || adminKey !== process.env.ADMIN_SECRET_KEY) {
        auditLogger.log('ADMIN_UNAUTHORIZED_ACCESS', { providedKey: adminKey ? 'PROVIDED_BUT_INVALID' : 'MISSING', ip: req.ip }, undefined, undefined, req.ip, req.get('User-Agent'));
...
export function registerAdminRoutes(app) {
    ...
    app.post('/api/admin/students/bulk', async (req, res) => {
        try {
            ...
            auditLogger.log(AuditLogger.ACTIONS.ADMIN_BULK_ADD_STUDENTS, { added: results.added, skipped: results.skipped, errors: results.errors }, undefined, undefined, req.ip, req.get('User-Agent'));
            res.status(201).json({
                success: true,
